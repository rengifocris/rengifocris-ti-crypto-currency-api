...
# A Hapi.js API following Clean Architecture principles to get the currency pricesfrom binance API

## Getting started (< 2mn)

```
git clone git@github.com:rengifocris/rengifocris-ti-crypto-currency-api.git
cd rengifocris-ti-crypto-currency-api.git
npm install
npm test
npm start
```
...
------  ----------------------------------------------  -----------------------------------------------------------
GET     /average                                        Get currency pairs average for a symbol for N last lectures
GET     /documentation                                                                                             
PUT     /getBinanceAvgPrice                             Update all currency pairs                                  
GET     /pairs                                          List all currency pairs                                    
POST    /pairs                                          Create a currency pair          
```

### Consuming POST saving data
````
curl --location --request POST 'http://localhost:8000/pairs' \
--header 'Content-Type: application/json' \
--data-raw '{
"symbol": "ETHUSDT" //sample data
}'
````

### Consuming GET retrieve data
````
curl --location --request GET 'http://localhost:8000/pairs'
````

### Consuming GET retrive average of n lectures from binance API 
````
curl --location --request GET 'http://localhost:8000/average?symbol=BTCUSDT&lectures=12'
````

In a browser, open [http://localhost:8000](http://localhost:8000).
